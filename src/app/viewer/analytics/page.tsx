/**
 * @file viewer/analytics/page.tsx
 * @description Analytics-Seite für Viewer-Rolle (Geschäftsführung / IT & Datenschutz).
 *
 * Identische Inhalte wie /admin/analytics, jedoch:
 * - Auth-Check für Rolle "viewer" (nicht "admin")
 * - Zurück-Link zu /viewer statt /admin
 * - Lila Akzentfarbe (visuelle Differenzierung vom Admin-Bereich)
 *
 * Funktionen:
 * - KPI-Kacheln: Gesamttickets, Erledigungsrate, Dringend-Anteil, Ø-Durchsatz
 * - Kreisdiagramm: Kategorienverteilung
 * - Balkendiagramme: Dringlichkeit + 7-Tage-Durchsatz
 * - CSV-Export: Alle Tickets als Excel-kompatible Datei
 */
"use client";
...
import {
  TrendingUp, Download, ArrowLeft,
  Clock, CheckCircle2, AlertTriangle, Activity, Eye, LayoutDashboard,
} from "lucide-react";
...
/** Ampelfarben für Dringlichkeitsstufen */
...
/**
 * ViewerAnalyticsPage – Einstiegspunkt mit Auth-Guard für Viewer-Rolle.
 * Leitet zu /admin/login weiter wenn nicht als Viewer eingeloggt.
 */
export default function ViewerAnalyticsPage() {
  ...
  return <ViewerAnalyticsContent tickets={tickets} />;
}

/** Analytics-Inhalt: KPIs + Diagramme + CSV-Export */
function ViewerAnalyticsContent({ tickets }: { tickets: Ticket[] }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100">
      ...
      {/* Header – Lila Akzent für Viewer-Bereich */}
      <header className="bg-white/80 backdrop-blur-xl border-b border-gray-200/60 sticky top-0 z-50 px-6 py-3.5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Link href="/viewer" className="flex items-center gap-2 text-gray-400 hover:text-gray-700 transition-colors mr-2" aria-label="Zurück zur Übersicht">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400 font-medium tracking-wide uppercase">Geschäftsführung / IT · Lesezugriff</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {/* CSV-Export (auch für Viewer erlaubt) */}
            <button
              onClick={exportCSV}
            ...
            {/* Read-only Hinweis */}
            <div className="hidden sm:flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-xl px-3 py-1.5 text-xs text-amber-600 font-medium">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KpiCard label="Tickets gesamt"  value={total}                    sub="alle Meldungen"           icon={<Activity className="w-5 h-5" />}     color="blue"  />
          <KpiCard label="Erledigungsrate" value={`${erledigungsRate}%`}    sub={`${erledigt} von ${total} erledigt`} icon={<CheckCircle2 className="w-5 h-5" />} color="green" />
          <KpiCard label="Dringend (Hoch)" value={hoch}                     sub="sofort priorisieren"      icon={<AlertTriangle className="w-5 h-5" />} color="red"   />
          <KpiCard label="Durchsatz Ø/Tag" value={(total / 7).toFixed(1)}   sub="letzte 7 Tage"            icon={<Clock className="w-5 h-5" />}         color="amber" />
        </div>

        {/* Diagramme: Kategorien + Dringlichkeit */}
        ...
        {/* Balkendiagramm: 7-Tage-Durchsatz */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wider mb-4 flex items-center gap-2">
...
/** KPI-Kachel Komponente */
function KpiCard({ label, value, sub, icon, color }: {
  label: string; value: string | number; sub: string;
  icon: React.ReactNode; color: "blue" | "green" | "red" | "amber";
}) {
  const colors = {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-shadow">
      <div className={`w-10 h-10 ${c.bg} ${c.text} rounded-xl flex items-center justify-center mb-3`}>{icon}</div>
